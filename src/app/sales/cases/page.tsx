...
export default function SalesCasesPage() {
  ...
  const [editingCase, setEditingCase] = useState<SalesCase | null>(null);
  ...
  // モーダルを開く（編集）
  const handleEdit = (salesCase: SalesCase) => {
    setEditingCase(salesCase);
    setFormData({
      title: salesCase.title,
      customerName: salesCase.customerName,
      customerCompany: salesCase.customerCompany || '',
      status: salesCase.status,
      stage: salesCase.stage || '',
      ...
      tags: salesCase.tags || [],
      assignedTo: salesCase.assignedTo || '',
      assignedToName: salesCase.assignedToName || ''
    });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(editingCase ? '案件を更新しました' : '案件を作成しました');
      setShowModal(false);
      resetForm();
      fetchCases();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = await user.getIdToken();
      const response = await fetch(`/api/sales/cases?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">{salesCase.customerName}</p>
                      {salesCase.customerCompany && (
                        <p className="text-xs text-gray-500 mt-1">{salesCase.customerCompany}</p>
                      )}
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(salesCase.status)}`}>
                  ...
                  <div className="space-y-2 mb-4">
                    {salesCase.estimatedValue && (
                      <div className="flex items-center text-sm text-gray-600">
                        <span className="font-medium mr-2">見積:</span>
                        <span>{salesCase.estimatedValue.toLocaleString()}円</span>
                      </div>
                    )}
                    {salesCase.probability && (
                      <div className="flex items-center text-sm text-gray-600">
                        <span className="font-medium mr-2">確率:</span>
                        <span>{salesCase.probability}%</span>
                      </div>
                    )}
                    {salesCase.expectedCloseDate && (
                      <div className="flex items-center text-sm text-gray-600">
                        <span className="font-medium mr-2">予定日:</span>
                        <span>{new Date(salesCase.expectedCloseDate).toLocaleDateString('ja-JP')}</span>
                      </div>
                    )}
                  </div>

                  {salesCase.tags && salesCase.tags.length > 0 && (
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
                  ...
                  >
                    {isSaving ? '保存中...' : editingCase ? '更新' : '作成'}
                  </button>
                </div>
              </div>
            </div>
