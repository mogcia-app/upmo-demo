...
export async function GET(request: NextRequest) {
  try {
    ...
    // 同じcompanyNameのユーザーまたは所有者のみ閲覧可能
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { customerId, title, meetingDate, meetingTime, location, attendees, assignee, actionItems, notes } = await request.json();

    if (!customerId || !title) {
      return NextResponse.json(
        { error: '顧客IDとタイトルは必須です' },
        { status: 400 }
      );
    }
    ...
    // 同じcompanyNameのユーザーまたは所有者のみ作成可能
    const userDoc = await db.collection('users').doc(userId).get();
    const userData = userDoc.data();
    const userCompanyName = userData?.companyName || '';
